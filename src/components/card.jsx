...
function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  const cardTexture = useTexture('/image.jpg')

  cardTexture.wrapS = THREE.RepeatWrapping
  cardTexture.wrapT = THREE.RepeatWrapping

  cardTexture.flipY = false 
  cardTexture.repeat.set(2.5, 2.5)
  cardTexture.offset.set(0.22, 0.12) 
  cardTexture.repeat.set(1, 0.9)
    ...
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[1, 1.05, 0.01]} />
          <group
            scale={2.25}
            position={[0, -1.2, -0.05]}
      ...
      <mesh ref={band}>
        <meshLineGeometry />
        <meshLineMaterial color="black" depthTest={false} resolution={[width, height]}  repeat={[-3, 1]} lineWidth={0.45} />
      </mesh>
    </>
  )
}